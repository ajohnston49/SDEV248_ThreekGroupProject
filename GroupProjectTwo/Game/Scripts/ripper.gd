	...
	if $AnimatedSprite2D.animation == "attack":
		...
		if player_ref and global_position.distance_to(player_ref.global_position) <= attack_range:
			if player_ref.has_method("take_damage"):
				# Push‑back vector: away from Ripper
				var knockback = (player_ref.global_position - global_position).normalized() * 50.0
				player_ref.take_damage(1, knockback)

func take_damage(amount: int = 1) -> void:
	health -= amount
	print("Ripper took damage. Health =", health)
	if health <= 0:
		queue_free()
